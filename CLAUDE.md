...
🚨 绝对禁止: 基于假设或猜测的架构决策
✅ 正确做法: 使用 `gm "分析[模块]架构冲突"` 获取Gemini深度分析，等待分析完成
```
...
### ⚠️ **编码完成质量审查铁律** - 🚨 质量生命线！
```cmd
代码编写完成 → 🛑 必须调用Gemini工具进行专业审查 → 质量合格 → 允许提交

🚨 绝对禁止: 未经AI专业审查直接提交代码
🚨 绝对禁止: 跳过安全审计环节
✅ 正确做法: 使用 `gm "审查[文件/模块]代码质量"` 获取专业审查报告
```
...
✅ 正确做法: 使用TodoWrite工具更新任务状态并同步项目文档
```
...
### 🔧 **AI协作工具自动调用规则** (🚨 Claude实例行为约束)
```cmd
**强制场景识别和自动执行**:

📊 **系统部署前** → 自动建议: `gm "分析系统健康状态"`
🤖 **技术问题求助** → 自动建议: `gm "分析具体技术问题"`
📚 **代码审查需求** → 自动建议: `gm "审查代码质量和安全性"`
```
**Claude实例执行原则**:
- ✅ **主动提醒**: 检测到适用场景时主动提醒用户使用Gemini工具
- ✅ **协助执行**: 用户同意后协助执行Gemini分析工作流
...
### 🔧 **核心工具**
 阅读项目根目录下GM_QUICK_GUIDE.md使用说明，掌握工具使用方法。
 铁律:使用gemini工具时，请使用Bash(gm指令) timeout: 20m命令语法，避免超时问题！严重注意timeout参数应该放在Bash工具的参数里，而不是命令内部,要放到括号外面，例如Bash(命令)timeout:20m,时间必须设置为10m以上。
 请在编码过程中经常咨询gemini的意见！
 严重注意：gemini对系统没有任何理解，每次调用都是一个新的实例，都需要让它扫描相关代码，给够足够的上下文才能给出实用建议！请在提示词中给它足够的信息，这样它才能给出有建设性的意见。
### 🎯 **编码前AI协作流程** (🚨 强制执行!)
...
#### **第二步: 深度代码分析和优化建议** (🤖 Gemini专业协作)
```bash
# 使用Gemini进行专业代码分析
gm "分析代码并给出具体修改建议"           # Gemini深度分析
gm "优化性能瓶颈的具体方案"               # Gemini性能建议
gm "重构代码的详细步骤"                   # Gemini重构方案
gm "修复安全漏洞的具体代码"               # Gemini安全修复

# 💡 注意: Gemini只提供建议，不执行修改
# ✅ 正确流程: Gemini分析 → 获取建议 → Claude手动实施
```
...
4️⃣ Claude实施代码 → 5️⃣ 本地编译验证 → 6️⃣ 用户重启服务 → 7️⃣ API功能测试
```

### 🔧 **调试协作** & **质量标准**
- **Debug模式**: 收集错误日志+代码上下文 → Gemini分析建议 → Claude实施修复
...
| 工具 | 角色定位 | 核心职责 | 权限范围 |
|------|----------|----------|----------|
| **Gemini CLI** | 📊 项目分析师 | 全项目扫描、架构分析、代码审查、性能优化建议 | 只读 |
| **Claude Code** | 🔨 实施工程师 | 代码编写、文件修改、命令执行、系统集成 | 读写 |

#### **标准工作流程**
```
1. Gemini扫描发现问题 → 2. Gemini分析给出建议 → 3. Claude实施修改 → 4. Gemini验证效果
```

#### **具体使用场景**
- **需要了解项目架构？** → `gm "分析项目架构"`
- **需要代码修改建议？** → `gm "如何优化这段代码"`
- **需要执行修改操作？** → Claude直接编辑文件
- **需要验证修改效果？** → `gm "验证修改是否成功"`

#### **🚨 核心原则**
- **Gemini提供分析和建议，Claude执行实施**
- **分析、审查、优化建议交给Gemini工具**
- **文件修改和命令执行由Claude负责**
- **双工具协同，职责明确**
...
#### **1. 🤖 编码建议 (Gemini CLI)** (⭐ 主要编程搭子)
```bash
# 推荐方式: 使用gm命令调用Gemini获取编码建议
gm "分析Controller安全风险并给出修复建议"   # 安全分析
gm "重构Service层的具体步骤"              # 架构重构
gm "优化数据库查询的详细方案"             # 性能优化
gm "修复Bug的具体代码修改"                # Bug修复

# 💡 分工明确:
# - Gemini CLI: 提供分析和建议 (只读)
# - Claude Code: 实施修改和测试 (读写)

# ✅ 工作流程:
# 1. gm分析 → 2. 获取建议 → 3. Claude实施 → 4. 编译测试
```
